...
app = FastAPI(title="Atlas T7 Python Market Service", version="0.1.0")
...
async def get_crypto_news(
    ...
    if mode == "single":
        if not sources or len(sources) != 1:
            raise HTTPException(status_code=400, detail="mode=single requires a single source parameter")
